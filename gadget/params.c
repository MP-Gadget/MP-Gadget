...
create_gadget_parameter_set()
{
    ...
    param_declare_double(ps, "PairwiseActiveFraction", OPTIONAL, 5e-8, "Pairwise gravity instead of tree gravity is used if N(active particles) / N(particles) is less than this.");
    ...
    param_declare_double(ps, "BlackHoleAccretionFactor", OPTIONAL, 1, "BH accretion boosting factor relative to the rate from the Bondi accretion model.");
