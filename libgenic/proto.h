...
int setup_grid(PetaPM * pm, double shift, int Ngrid, double mass, int NumPart, struct ic_part_data * ICP);
...
int setup_glass(PetaPM * pm, double shift, int Ngrid, int seed, double mass, int NumPart, struct ic_part_data * ICP);
...
int get_size_offset(PetaPM * pm, int * size, int * offset, int Ngrid);

/* Works out the id of a particle from its index and the processor number*/
uint64_t id_offset_from_index(PetaPM * pm, const int i, const int Ngrid);
