...
static void save_transfer(BigFile * bf, int ncol, struct table * ttable, const char * bname, int ThisTask, const char * colnames[])
{
    ...
    _bigfile_utils_create_block_from_c_array(bf, ttable->logk, buf, "f8", dims, sizeof(double), 1, MPI_COMM_WORLD);

    for(i = 0; i < ncol; i++)
    {
        snprintf(buf, 100, "%s/%s", bname, colnames[i]);
        _bigfile_utils_create_block_from_c_array(bf, ttable->logD[i], buf, "f8", dims, sizeof(double), 1, MPI_COMM_WORLD);
    }
}
