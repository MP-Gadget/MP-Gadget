...
metal_return_copy(int place, TreeWalkQueryMetals * input, TreeWalk * tw)
{
    ...
    input->MetalGenerated = InitialMass * metal_yield(dtmyrstart, dtmyrend, input->Metallicity, METALS_GET_PRIV(tw)->hub, &METALS_GET_PRIV(tw)->interp, input->TotalMetalGenerated, METALS_GET_PRIV(tw)->imf_norm, METALS_GET_PRIV(tw)->gsl_work[tid]);
    /* Fix units of metal generated*/
    int i;
    for(i = 0; i < NMETALS; i++)
        input->TotalMetalGenerated[i] *= InitialMass;
}
