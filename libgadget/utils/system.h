...
/* Check the version of OPENMP. We need OpenMP 4.0 atomic captures and
 * OpenMP 4.5 array reduction clauses. Earlier compilers, especially
 * gcc 4.7 and 4.8 (which are unfortunately widely deployed as default compilers)
 * have buggy support for OpenMP anyway. Note that clang supports all non-offloading
 * OpenMP 4.5 features before advertising OpenMP 4.5 so you may be able to get away with earlier versions.*/
#if _OPENMP < 201511
#error MP-Gadget requires OpenMP >= 4.5. Use a newer compiler (gcc >= 6, intel >= 17 clang >= 8).
#endif
