...
fof_compile_base_2(struct BaseGroup * base, int NgroupsExt, struct fof_particle_list * HaloLabel, MPI_Comm Comm)
{
    ...
    for(i = 0; i < NgroupsExt; i++)
    {
        ...
        for(;start < PartManager->NumPart; start++) {
            ...
            base[i].LenType[P[HaloLabel[start].Pindex].Type]++;
            base[i].Length ++;
        }
    }
...
void
fof_save_groups(FOFGroups * fof, const char * OutputDir, const char * FOFFileBase, int num, double FOFPartAllocFactor, Cosmology * CP, double atime, const double * MassTable, int StarformationOn, int BlackholeOn, MPI_Comm Comm)
{
    fof_save_particles(fof, OutputDir, FOFFileBase, num, fof_params.FOFSaveParticles, FOFPartAllocFactor, CP, atime, MassTable, StarformationOn, BlackholeOn, Comm);
}
