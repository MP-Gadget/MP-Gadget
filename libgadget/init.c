...
inttime_t init(int RestartSnapNum, DomainDecomp * ddecomp)
{
    ...
    if (RestartSnapNum < 0) {
        ...
        setup_sync_points(All.TimeIC, All.TimeMax, All.ExcursionSetZStop, All.UVBGTimestep, 0.0, All.SnapshotWithFOF);
    } else {
        /* skip dumping the exactly same snapshot */
        setup_sync_points(All.TimeIC, All.TimeMax, All.ExcursionSetZStop, All.UVBGTimestep, All.TimeInit, All.SnapshotWithFOF);
        ...
        if(round_down_power_of_two(ti_init) != ti_init) {
            ...
            setup_sync_points(All.TimeInit, All.TimeMax, All.ExcursionSetZStop, All.UVBGTimestep, All.TimeInit, All.SnapshotWithFOF);
        }
    }
