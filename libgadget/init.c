...
setup_density_indep_entropy(const ActiveParticles * act, ForceTree * Tree, double u_init, double a3)
{
    ...
    for(j = 0; j < 100; j++)
    {
        ...
        density(act, 0, All.DensityIndependentSphOn, All.BlackHoleOn, All.WindOn, All.HydroCostFactor * All.cf.a, Tree);
        if(stop)
            break;
...
setup_smoothinglengths(int RestartSnapNum, DomainDecomp * ddecomp)
{
    ...
    density(&act, 1, 0, All.BlackHoleOn, All.WindOn, All.HydroCostFactor * All.cf.a,  &Tree);
