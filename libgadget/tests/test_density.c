...
static void do_density_test(void ** state, const int numpart, double expectedhsml, double hsmlerr)
{
    ...
    ForceTree tree = {0};
    force_tree_rebuild(&tree, &ddecomp, PartManager->BoxSize, 0, 1, NULL);
    set_init_hsml(&tree);
    /* Rebuild without moments to check it works*/
    force_tree_rebuild(&tree, &ddecomp, PartManager->BoxSize, 0, 0, NULL);
...
static int setup_density(void **state) {
    ...
    for(i = 0; i < 6; i++)
        maxpart+=atleast[i];
    double BoxSize = 8;
    particle_alloc_memory(maxpart, BoxSize);
